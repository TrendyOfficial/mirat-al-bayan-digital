...
export default function DeletionReviews() {
  ...
  const publicationReviews = reviews.filter(r => r.item_type === 'publication');
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Badge variant="secondary">
          {reviews.length} {isArabic ? 'طلب قيد الانتظار' : 'pending requests'}
        </Badge>
      </div>
      ...
      {reviews.length === 0 && (
        <Card>
          <CardContent className="p-8 text-center text-muted-foreground">
            {isArabic ? 'لا توجد طلبات حذف قيد المراجعة' : 'No pending deletion requests'}
          </CardContent>
        </Card>
      )}
                ...
                >
                  <div>
                    <p className="font-medium">
                      {isArabic ? review.item_data.name_ar : review.item_data.name_en}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {isArabic ? 'طلب من' : 'Requested by'}: {review.requested_by_email}
                    </p>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleReject(review.id, review.item_type)}
                    >
      ...
      {publicationReviews.length > 0 && (
        <Card className="border-yellow-500">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-yellow-600">
              {isArabic ? 'طلبات حذف المقالات' : 'Publication Deletion Requests'}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {publicationReviews.map((review) => (
                ...
                >
                  <div>
                    <p className="font-medium">
                      {isArabic ? review.item_data.title_ar : (review.item_data.title_en || review.item_data.title_ar)}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {isArabic ? 'طلب من' : 'Requested by'}: {review.requested_by_email}
                    </p>
